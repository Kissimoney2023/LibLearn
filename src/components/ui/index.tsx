...
import {PROVENANCE_LABEL, type ContentProvenance} from '../../types/domain';
...
/**
 * Says where a piece of content came from.
 *
 * This badge is the student-facing half of the provenance system in
 * src/data/sources.ts. It must appear wherever curriculum content is shown, so
 * that nothing LibLearn wrote can be mistaken for something the Ministry of
 * Education or WAEC published.
 */
export function ProvenanceBadge({
  ...
  const style: Record<ContentProvenance, string> = {
    official: 'border-primary text-primary',
    verified: 'border-primary text-primary',
    liblearn: 'border-outline text-on-surface-variant',
    'ai-generated': 'border-tertiary-container text-on-surface-variant',
  };
  const title: Record<ContentProvenance, string> = {
    official:
      'Reproduced from an official government, Ministry or WAEC document.',
    verified: 'Supported by a credible published educational source.',
    liblearn:
      'Teaching material written for LibLearn. Academically standard, but NOT ' +
      'the official Liberian curriculum and not a statement of what is on your examination.',
    'ai-generated':
      'Generated by the AI tutor. Check it against your lesson or teacher before relying on it.',
  };
  return (
    <span
      className={`inline-flex h-7 items-center rounded-full border px-3 text-xs font-medium ${style[provenance]}`}
...
/** @deprecated Use <ProvenanceBadge provenance="liblearn" /> instead. */
export function DemoBadge() {
  return <ProvenanceBadge provenance="liblearn" />;
}
